...
<div id="type_list" class="wrap">
	...
	<ul>
		<?php foreach ($list as $k => $v) { ?>
			<li onclick="getProgram(this,<?=$v['id']?>)"><?=$v['lv']?str_repeat('│└',$v['lv']):'├';?><?=$v['title']?></li>
		<?php } ?>
	</ul>
</div>
...
$(function(){
	...
	function getProgram(obj,id){
		...
		$.ajax({
			url:'index.php?d=admin&c=programme&m=getList&id='+id,
			type:'get',
			success:function(res){
				...
				for (var i in json) {
					...
					html += '<li style="overflow: hidden;" class="'+has_selected+'" data-id="'+json[i].id+'" onclick="select_one(this,'+json[i].id+',\''+json[i].title+'\')">'+json[i].title+'</li>';
				}
