	...
	<form action="<?=$this->baseurl?>&m=delete" method="post" id="my_form">
		...
		<table class="tablelist">
			...
			<?php foreach($list as $key=>$r) {?>
				<tr>
					<td><input type="checkbox" name="dir[]" value="<?=$r['name']?>" class="checkbox" /></td>
					<td><a title="点击进入" href="<?=$this->baseurl?>&m=import&url=<?=$r['name']?>" ><?=$r['name']?></a></td>
					<td>
						<a title="点击进入" href="<?=$this->baseurl?>&m=import&url=<?=$r['name']?>" >进入文件夹</a>
						&nbsp;&nbsp;
						<?php if(checkAccess('database_del')){?>
							&nbsp;&nbsp;<a href="<?=$this->baseurl?>&m=delete&dir=<?=$r['name']?>" onclick="return confirm('确定要删除吗？');">删除</a>
						<?php  }else{echo '--';}?>
					</td>
				</tr>
			<?php }?>
