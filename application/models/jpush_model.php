...
// 		->setNotification(M\notification($data['notification']))
	...
	function  jpush_private($data) {		
		...
		$data ['to_uid'] = array($data['to_uid']);
		try {
			$client = new JPushClient ( $this->jpush ['appKey'], $this->jpush ['masterSecret'] );
			/*$result = $client->push ()->setPlatform ( M\all )			// 			->setPlatform ( M\platform("ios") )
			->setAudience ( M\audience ( M\alias ( $data ['to_uid'] ) ) )
			->setMessage ( M\message ( $data['addtime'], $data['title'], "content_type", $data) )
			->send ();*/
			$client = new JPushClient ( $this->jpush ['appKey'], $this->jpush ['masterSecret'] );
			$result = $client->push()->setPlatform(M\all)
			->setAudience ( M\audience ( M\alias ( $data ['to_uid'] ) ) )               
...
/**
	...
	function jpush_group($data) {		
		...
		try {
			$client = new JPushClient ( $this->jpush ['appKey'], $this->jpush ['masterSecret'] );
			$result = $client->push()->setPlatform(M\all)
			->setAudience ( M\audience ( M\tag ( $tagList )))                      
			->setMessage(M\message( $data['addtime'], $data['title'], "content_type", $data ))
            ->send();
			print_r ( $result );
		} catch ( APIRequestException $e ) {
	 ...
	 function testMessageContentFull($data) {
		 $tagList = array($data['group_tag']);
        $client = new JPushClient($this->jpush ['appKey'], $this->jpush ['masterSecret']);
	  ...
	  public function testAlertAll($data) {
        $client = new JPushClient($this->jpush ['appKey'], $this->jpush ['masterSecret']);
        $response = $client->push()->setPlatform(M\all)
            ->setAudience(M\all)
            ->setNotification(M\notification($data['title']))
