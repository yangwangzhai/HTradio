...
class groups_model extends CI_Model
{
    ...
    {
    	$result = array();
    	$query = $this->db->query("select id,title from  fm_groups order by id limit 200");
...
//     	$query = $this->db->query("select id,title,keywords,thumb,last_time,last_title from fm_groups where hot=1 ORDER BY type desc,last_time desc limit 30");
    	if( $uid > 0 ){
			$query = $this->db->query("select id,title,keywords,thumb,last_time,last_title,uid,type,url,isbase64,status from fm_groups where hot=1 AND (status=1 OR uid=$uid )  ORDER BY sort,last_time desc limit 100");
		}else{				
			$query = $this->db->query("select id,title,keywords,thumb,last_time,last_title,uid,type,url,isbase64,status from fm_groups where hot=1 AND status=1 ORDER BY sort,last_time desc limit 100");
				}
    ...
    {
    	$query = $this->db->query("select id from fm_groups where title='$title' limit 1");
    ...
    {
    	$query = $this->db->query("select * from fm_groups where id='$id' limit 1");
    	$value = $query->row_array();
    	$value['group_tag'] = 'group'.$value['id'];
    	return $value;
    }
    ...
    function memberid_for_group($groupid) { 	
    	$query = $this->db->query("select title from fm_groups where id='$groupid' limit 1");
    	$value = $query->row_array();
    	$grouptitle = $value['title'];
    	$query = $this->db->query("select id from fm_member where nickname like '%$grouptitle%' limit 1");
    	$value = $query->row_array();
    	$uid = $value['id'];
    	return $uid;
    }
    ...
    function chats_noread($groupid, $last_time) {    	
    	$query = $this->db->query ( "SELECT COUNT(*) AS num FROM fm_groups_chats where addtime>$last_time" );
    	$value = $query->row_array ();
    	return $value ['num'];
    }
    ...
    function chats_save($data) {
    	...
    	// 保存到群聊表
    	$this->db->insert ( 'fm_groups_chats', $data);
    ...
    function check_weibo($id) {
    	...
    	$query = $this->db->query ( 'select id from fm_weibo where id='.$id.' limit 1');
    	$value = $query->row_array();
    	if(empty($value)) {
    		$this->db->insert('fm_weibo', array('id'=>$id));
    		$result = false;
    	}
    ...
    {
    	$query = $this->db->query("select * from fm_groups_chats where id='$id' limit 1");
    	$value = $query->row_array();  
    ...
    {
    	$this->db->where('id', $id);
    	$this->db->delete('fm_groups_chats');
