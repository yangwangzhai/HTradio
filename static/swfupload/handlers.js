...
function uploadSuccess(file, serverData) {
	try {
		...
		if (serverData.indexOf("suc")!=-1) {
			...
			var url=fileinfo[0].substr(1);
			var progress = new FileProgress(file, this.customSettings.progressTarget);
			progress.setComplete();
			progress.setText("<a href=\""+getRootPath()+fileinfo[0]+fileinfo[1]+"\" target=\"_blank\">"+fileinfo[1]+"</a>");
			var status = "恭喜你，文件上传成功！ <br />";
			status += "大小："+parseFloat(parseInt(fileinfo[2])/1024).toFixed(2)+ "K ";
			status += "<br />地址："+getRootPath()+fileinfo[0]+fileinfo[1];  //这里需要自己根据自己网站实际情况坐下修改
			$("#login-form-path").val(url+fileinfo[1]);//文件上传成功后，将文件地址赋值给input输入框
			progress.setStatus(status);
			progress.toggleCancel(false);
			//document.getElementById('videourl').value = getPath(fileinfo[0],fileinfo[1]);
			document.getElementById('file').value =  getPath(fileinfo[0],fileinfo[1]);
			document.getElementById('videourl').style.border = "1px solid #c3c3d6";
			document.getElementById('videourl').nextSibling.nextSibling.style.display = "none";
