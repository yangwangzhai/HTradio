...
//  Embedding flash object ---------------------------------------------------------------------------------------------
  ...
  var appWidth = 24;
  var appHeight = 24;
  var flashvars = {'upload_image': 'static/rec/html/images/upload.png'};
  var params = {};
  var attributes = {'id': "recorderApp", 'name': "recorderApp"};
  swfobject.embedSWF("static/rec/html/recorder.swf", "flashcontent", appWidth, appHeight, "11.0.0", "", flashvars, params, attributes);

//  Handling FWR events ------------------------------------------------------------------------------------------------

  window.fwr_event_handler = function fwr_event_handler() {
    //$('#status').text("最后执行的事件: " + arguments[0]);
    var name, $controls;
    switch (arguments[0]) {
      case "ready":
      ...
      case "recording_stopped":
        ...
        $('#duration').text(duration.toFixed(2) + "秒");
        break;
      ...
      case "saved":
        ...
        if (data.saved) {
			...
			//window.location.href="index.php?c=upload&m=upload_rec&url="+data.url;
         //$('#path').value(data.url);
		
        } else {
          $('#upload_status').css({'color': '#F00'}).text(" 上传失败");
        }
        break;
